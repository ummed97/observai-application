...
import {
  ...
  Filter, Layers, Cuboid
} from 'lucide-react';
import Layout from '../components/common/Layout';
import Topology3D from '../components/topology/3DGraph';
...
export const TopologyView: React.FC = () => {
  ...
  const [filterOptions, setFilterOptions] = useState({
    ...
    connectors: [] as string[] // In a real app, fetch names
  });
  ...
  const fetchTopologyData = async () => {
    try {
      ...
      if (filters.subscriptionId) queryParams.append('subscription_id', filters.subscriptionId);
      if (filters.resourceGroup) queryParams.append('resource_group', filters.resourceGroup);
      if (filters.connectorId) queryParams.append('connector_id', filters.connectorId);

      const response = await fetch(`${API_BASE}/api/v1/topology/graph?${queryParams}`, { headers });
      ...
      if (!filters.subscriptionId && !filters.resourceGroup && !filters.connectorId) {
        ...
        setFilterOptions({
          subscriptions: subs,
          resourceGroups: rgs,
          connectors: [] // TODO: Populate from API or node data
        });
      }
          ...
          <div className="p-4 space-y-4 flex-1 overflow-y-auto">
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => setFilters(prev => ({ ...prev, subscriptionId: e.target.value, resourceGroup: '' }))}
              >
            ...
            <div>
              <label className="block text-xs font-medium text-gray-500 mb-1">Resource Group</label>
              <select
                className="w-full text-sm border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500"
                value={filters.resourceGroup}
                onChange={(e) => setFilters(prev => ({ ...prev, resourceGroup: e.target.value }))}
              >
                <option value="">All Resource Groups</option>
                {filterOptions.resourceGroups.map(rg => (
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex bg-gray-200 rounded-lg p-1">
                <button
                  ...
                  className={`p-1.5 rounded-md transition-all ${viewMode === '2d' ? 'bg-white shadow-sm text-blue-600' : 'text-gray-500 hover:text-gray-700'}`}
                  title="2D View"
                >
                  <Layers size={16} />
                </button>
                <button
                  ...
                  className={`p-1.5 rounded-md transition-all ${viewMode === '3d' ? 'bg-white shadow-sm text-blue-600' : 'text-gray-500 hover:text-gray-700'}`}
                  title="3D View"
                >
                  <Cuboid size={16} />
                </button>
